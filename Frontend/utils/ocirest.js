...
function sign(auth, request, body) {
  ...
  var newAuthHeaderValue = request
...
function handleResponse(callback) {
  return function(response) {
    ...
    response.on("data", function(chunk) {
      ...
      if (contentType.includes("application/json")) JSONBody += chunk;
      if (contentType.includes("application/x-www-form-urlencoded"))
        buffer.push(Buffer.from(chunk, "binary"));
      if (contentType.includes("application/octet-stream")) buffer.push(chunk);
    });
  ...
  for (var i = 0; i < possibleQuery.length; i++)
    if (possibleQuery[i] in options)
      query +=
        ...
        possibleQuery[i] +
        "=" +
        encodeURIComponent(options[possibleQuery[i]]);
  return query;
}
